	...
	<body>
		...
		<section>
			...
			<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" autocomplete="off">
				...
				<br>
				Equation Order: <input type="number" name="eqOrder" required>
				<br>
			...
			<?php
				if ($_SERVER["REQUEST_METHOD"] == "POST") {
					...
					$eqOrder = mysqli_real_escape_string($link,$_POST['eqOrder']);
					$_SESSION['flag'] = 0;
					...
					if (($link->query("SELECT * FROM Equations WHERE name='$name'"))->num_rows == 0) {
						if (($link->query("SELECT * FROM Equations WHERE latexCode='$latexCode'"))->num_rows == 0) {
							...
							$sql2 = "INSERT INTO Differential_equations (deid, eqOrder) VALUES ($id, $eqOrder)";
							if(!$link->query($sql1) || !$link->query($sql2)) {
								header('Location: http://clabsql.clamv.jacobs-university.de/~jwhiteley/feedbackPage.php');
								$_SESSION['flag'] = 1;
